...
public class TreePanel extends JPanel {
    ...
    private static final int TIP_WIDTH = 180;
    private static final int TIP_HEIGHT = 140;
    private Node root;
    ...
    public TreePanel() {
        ...
        this.setBounds(0, 0, TIP_WIDTH, TIP_HEIGHT);
        this.add(tipPanel);
    }
    ...
    public void changeSize(int delta) {
        ...
        this.setSize(Main.WIDTH * sizeMultiplier, Main.HEIGHT * 4);
    }

    @Override
    public Dimension getPreferredSize() {
        return new Dimension(Main.WIDTH * sizeMultiplier, Main.HEIGHT * 4);
    }
    ...
    /**
     * The method that gets called when the user moves the mouse. This will find the
     * node moved over and print information on the node.
     *
        ...
        if (queue != null && queue.size() > 0) {
            for (DrawInfo node : queue) {
                if (node.inside(x, y)) {
                    ...
                    this.tipPanel.setBounds(x, y, TIP_WIDTH, TIP_HEIGHT);
    ...
    /**
     * The method that gets called when the user clicks the mouse. This will find
     * the node clicked on and print information on the node.
     *
     * @param me The MouseEvent data structure provided by the event.
     */
    private void onMouseClicked(MouseEvent me) {
        ...
        if (queue != null && queue.size() > 0) {
            for (DrawInfo node : queue) {
                if (node.inside(x, y)) {
                    if (singleClick) {
                        node.node.clickEvent(me.getModifiersEx());
                    } else {
    ...
    // Second Pass balancing will start near the bottom of the tree (2nd to last
    // row).
    // Start at the right and make the parent be the average rowOrdinal of its
    // children.
